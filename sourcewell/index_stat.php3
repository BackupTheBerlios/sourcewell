...
."<a href=\"".$sess->url("index.php3").$sess->add_query(array("start" => strftime("%Y-%m-%d", $time), "days" => $days, "by" => "Date"))."\">".$t->translate("Date")."</a>"
." | <a href=\"".$sess->url("index.php3").$sess->add_query(array("start" => strftime("%Y-%m-%d", $time), "days" => $days, "by" => "Importance"))."\">".$t->translate("Importance")."</a>"
." | <a href=\"".$sess->url("index.php3").$sess->add_query(array("start" => strftime("%Y-%m-%d", $time), "days" => $days, "by" => "Urgency"))."\">".$t->translate("Urgency")."</a>"
." | <a href=\"".$sess->url("index.php3").$sess->add_query(array("start" => strftime("%Y-%m-%d", $time), "days" => $days, "by" => "Name"))."\">".$t->translate("Name")."</a>\n";

$bs->box_strip($sort);
$nav = "<a href=\"".$sess->url("index.php3").$sess->add_query(array("start" => strftime("%Y-%m-%d", $weekprev), "days" => $days, "by" => $by))."\">&lt;&nbsp;".$t->translate("Week")."</a>"
." | <a href=\"".$sess->url("index.php3").$sess->add_query(array("start" => strftime("%Y-%m-%d", $timeprev), "days" => $days, "by" => $by))."\">&lt;&nbsp;".$t->translate("Day")."</a>"
." | <a href=\"".$sess->url("index.php3").$sess->add_query(array("start" => strftime("%Y-%m-%d", $time), "days" => $days, "cnt" => $prev_cnt, "by" => $by))."\">&lt;&nbsp;$config_show_appsperpage ".$t->translate("Apps")."</a>"
." | <a href=\"".$sess->url("index.php3").$sess->add_query(array("by" => "Date"))."\">".$t->translate("Today")."</a>";

if ($cnt > 0) {
  $nav .= " | <a href=\"".$sess->url("index.php3").$sess->add_query(array("start" =>strftime("%Y-%m-%d", $time), "days" => $days, "cnt" => $next_cnt, "by" => $by))."\">$config_show_appsperpage&nbsp;".$t->translate("Apps")."&nbsp;&gt;</a>";
} else {
  $nav .= " | $config_show_appsperpage&nbsp;".$t->translate("Apps")."&nbsp;&gt;";
}

if ($timenext < time()) {
  $nav .= " | <a href=\"".$sess->url("index.php3").$sess->add_query(array("start" =>strftime("%Y-%m-%d", $timenext), "days" => $days, "by" => $by))."\">".$t->translate("Day")."&nbsp;&gt;</a>";
} else {
  $nav .= " | ".$t->translate("Day")."&nbsp;&gt;";
}

if ($weeknext < time()) {
  $nav .= " | <a href=\"".$sess->url("index.php3").$sess->add_query(array("start" =>strftime("%Y-%m-%d", $weeknext), "days" => $days, "by" => $by))."\">".$t->translate("Week")."&nbsp;&gt;</a></b>";
} else {
